...
While there is disagreement on where its boundaries lie, a distinction between semantics and pragmatics is commonly agreed upon by linguists. Broadly put, semantics can be viewed as concerning the literal, truth-conditional, meaning of expressions whereas pragmatics concerns information conveyed by expressions that goes beyond its literal meaning. Consider, for instance, the following sentence

\begin{exe}
\ex SI sentence \begin{xlist}
	\ex Numeral n-1
	\ex SI
\end{xlist}
\end{exe}

%Note first, that the literal meaning of {\em NUMERAL} is compatible with that of more informative or ``stronger'' alternatives such as {\em n+1, n+1} and so on. 




\paragraph{Semantics-pragmaits distinction and where the puzzle arises.}

\paragraph{Description of plan to lay out model and test it on upper-bounds.}


\section{Cultural transmission \& linguistic pressures}

\paragraph{Overview of generally assumed linguistic pressures.} \hl{Focus has been put mainly on one over the other, even though it is mutually accepted that both are important. We involve both and connect this research to the rational probabilistic interlocutors literature}
