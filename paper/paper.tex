...
\hl{some general sentence here}
In linguistic theorizing, it is common to draw a distinction between semantics and pragmatics. Broadly speaking, semantics concerns the truth-conditional content of expressions, whereas pragmatics deals with information beyond such literal meanings. For instance, the integration of context and the derivation of defeasible inferences. Thus, the information conveyed is seldom if ever solely determined by semantics, but rather by its pragmatic enrichment. The present investigation addresses the selection of particular semantics over others in light of pragmatics. %  the semantics-pragmatics distinction raises the challenge of justifying the former's structure. 

Consider, for instance, the utterance {\em Bill read five books}. Truth-conditionally, this utterance does not set an upper-bound for the amount of books that Bill read; he may have read six, seven, or more books. Pragmatically, however, an addressee who assumes the speaker to be cooperative and knowledgeable may draw the (defeasible) inference that {\em Bill read no more than five books}. After all, had he read more, for example, six books, and the speaker had known this, she would have said {\em Bill read six books} instead. In this way, what is conveyed (pragmatically) goes beyond what is (literally) said \citep{grice:1975}. 

Much research in semantics and pragmatics has been aimed at characterizing the information conveyed by classes of expressions, such as numerals, in terms of either domain, or their interplay. Thus, while there is disagreement on where their boundary lies, their distinction remains a core (if tacit) assumption in linguistics. An issue that has received little attention, however, is the justification of semantic structure in view of pragmatics. Prima facie, under a functionalist view, is is puzzling that {\em Bill read five books} does not semantically rule out its more informative or ``stronger'' alternatives. Wouldn't it serve speakers of a language better if weak(er) expressions such as {\em warm}, {\em or}, {\em some} or {\em big} were not truth-conditionally compatible with stronger alternatives such as {\em hot}, {\em and}, {\em all} and {\em huge}, respectively? After all, this is what an interlocutor's pragmatically enriched interpretation yields in the case of so-called scalar inferences. Once a distinction between semantics and pragmatics is drawn, similar questions can be posed for other types of pragmatic inferences, such as ignorance inferences as (\ref{ex:ignorance}), or manner inferences such as (\ref{ex:manner-1}) and (\ref{ex:manner-2}).

\begin{exe}
\ex\label{ex:ignorance} \begin{xlist}
	\ex Bill read at least five books
	\ex $\leadsto$ Bill read five or more books (but the speaker doesn't know how many)
\end{xlist}
\ex \label{ex:manner-1} \begin{xlist}
	\ex Mercader killed Trotsky.
	\ex $\leadsto$ Mercader killed Trotsky in a stereotypical way
    \end{xlist}
\ex \label{ex:manner-2} \begin{xlist} \ex Mercader caused Trotsky to die
		  \ex $\leadsto$ Mercader killed Trotsky in a non-stereotypical way
    \end{xlist}
\end{exe}


In more general terms, the question then is what motivates the selection of particular semantic structures over other hypothetical alternatives in light of pragmatics. While a number of recent investigations have began to address issues pertaining to the development and selection of linguistic features, the semantics-pragmatics distinction has received little attention (see \citealt{steels:2015} and \citealt{tamariz+kirby:2016} for recent overviews). Our starting point is given by the overarching account of competing pressures that crystallizes across different frameworks: Natural languages need to allow for successful communication, one the one hand, and be well suited for acquisition on the other. Put differently, languages need to be well-adapted to the communicative needs within a linguistic community, and be learnable to survive their faithful transmission across generations. 

 \hl{Description of plan to lay out model and test it on upper-bounds.}


\hl{general notes on cultural evolution, biological evolution, and their connection}
...
\subsection{Probabilistic models of language use}
